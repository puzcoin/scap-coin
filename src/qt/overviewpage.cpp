...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SCAP)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("SCAP")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivatesendProgress()
{
    ...
    QString strAnonymizeSafeCapitalAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSafeCapitalAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSafeCapitalAmount = strAnonymizeSafeCapitalAmount.remove(strAnonymizeSafeCapitalAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSafeCapitalAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSafeCapitalAmount * COIN) nMaxToAnonymize = nAnonymizeSafeCapitalAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSafeCapitalAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSafeCapitalAmount));
        strAnonymizeSafeCapitalAmount = strAnonymizeSafeCapitalAmount.remove(strAnonymizeSafeCapitalAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSafeCapitalAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    } else {
...
void OverviewPage::togglePrivatesend()
{
    ...
    if (!fEnablePrivatesend) {
        ...
        if (nAnonymizeSafeCapitalAmount == 0) {
